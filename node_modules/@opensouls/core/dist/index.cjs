...
__export(src_exports, {
  ...
  SoulEventKinds: () => SoulEventKinds,
  WorkingMemory: () => WorkingMemory,
  createCognitiveStep: () => createCognitiveStep,
  debugChatShape: () => debugChatShape,
  eventLogShape: () => eventLogShape,
  extractJSON: () => extractJSON,
  forkStream: () => forkStream,
  getProcessor: () => getProcessor,
  indentNicely: () => indentNicely,
  prepareMemoryForJSON: () => prepareMemoryForJSON,
  registerProcessor: () => registerProcessor,
  stripEntityAndVerb: () => stripEntityAndVerb,
  stripEntityAndVerbFromStream: () => stripEntityAndVerbFromStream,
  z: () => import_zod.z
});
...
var import_gpt_4 = require("gpt-tokenizer/model/gpt-4");
...
var indentNicely = import_common_tags.codeBlock;
...
function forkStream(originalStream, count = 2) {
  const streams = Array.from({ length: count }, () => {
    ...
    const stream = new import_web_streams_polyfill.ReadableStream({
      start(c) {
...
// src/processors/OpenAIProcessor.ts
var tracer = import_api.trace.getTracer(
...
var tokenLength = (messagesOrContent) => {
  ...
  if (typeof messagesOrContent === "string") {
    for (const tokens of (0, import_gpt_4.encodeGenerator)(messagesOrContent)) {
  ...
  for (const tokens of (0, import_gpt_4.encodeChatGenerator)(messagesWithoutImages)) {
    tokenCount += tokens.length;
  }
  return tokenCount;
};
          ...
          async () => {
            ...
            if (opts.schema) {
              ...
              try {
                ...
                const zodError = (0, import_zod_validation_error.fromZodError)(err);
...
var tracer2 = import_api2.trace.getTracer(
  "open-souls-OpenAIProcessor",
  "0.0.1"
);
...
var AnthropicProcessor = class {
  ...
  constructor({ clientOptions, defaultRequestOptions, defaultCompletionParams, customClient }) {
    this.client = new (customClient ?? import_sdk.default)(clientOptions);
    this.defaultRequestOptions = defaultRequestOptions || {};
    this.defaultCompletionParams = defaultCompletionParams || {};
  }
  async process(opts) {
    return tracer2.startActiveSpan("OpenAIProcessor.process", async (span) => {
      ...
      return (0, import_exponential_backoff2.backOff)(
        async () => {
          const resp = await this.execute({
            ...opts,
            memory
...
var WorkingMemory = class _WorkingMemory extends import_eventemitter3.EventEmitter {
  ...
  * 
    ...
    try {
      ...
      if (schema && !skipAutoSchemaAddition) {
        commandMemory.content += "\n\n" + indentNicely`
          ...
          ${JSON.stringify((0, import_zod_to_json_schema.zodToJsonSchema)(schema), null, 2)}
        `;
      }
  ...
  memoriesFromInputMemories(memories) {
    return memories.map((memory) => {
      return {
        ...memory,
        _id: memory._id || (0, import_nanoid.nanoid)(),
